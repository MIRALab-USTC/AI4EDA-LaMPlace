...
def gen_dataset(train: True):
    ...
    for bench_id in bench_ids:
        results = np.loadtxt(f'./dataset/gp_results/superblue{bench_id}.txt',usecols=range(4))
        hpwl = results[:,0]
        n = min(len(hpwl),150)
        hpwl = hpwl[:n]
        con = results[:,1][:n]
        tns = results[:,2][:n]
        wns = results[:,3][:n]
        pl_dir = f'./dataset/placement/superblue{bench_id}/pl';
        #mp_file = getListOfFiles(pl_dir)
        #assert n == len(mp_file)
        pos = []
        for i in range(n):
            mp_file = f"./dataset/placement/superblue{bench_id}/result{i}.pl"
...
with open(f"dataset/dataset.pkl","wb") as f:
