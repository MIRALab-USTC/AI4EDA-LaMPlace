    ...
    def initialize_from_rawdb(self, params):
        ...
        if (not params.global_place_flag) and os.path.exists(params.aux_input):
            ...
            if filename is not None and os.path.exists(filename):
                ...
                self.node_orient = np.zeros(self.num_physical_nodes, dtype=np.str_)
        ...
        if not use_read_pl_flag:
            ...
            self.node_orient = np.array(pydb.node_orient, dtype=np.str_)
        ...
        self.pin_direct = np.array(pydb.pin_direct, dtype=np.str_)
        ...
        all_macro_ids = [i for i, b in enumerate(self.pydb.is_macro_list) if b]
        # pick 128 macros with max areas
        if params.design_name() == "ariane133" or params.design_name() == "ariane136":
            self.macro_ids = sorted(all_macro_ids, key=lambda x: self.node_size_x[x] * self.node_size_y[x], reverse=True)
        else:
            self.macro_ids = sorted(all_macro_ids, key=lambda x: self.node_size_x[x] * self.node_size_y[x], reverse=True)[:128]
